...
cleaning_prob <- function(opt) {
  ...
  averageProb <- function(data, conditions, states) {
    ...
    conditions_proba_list <-
      lapply(
        ...
        FUN = function(conditionN) {
          ...
          ## for each node
          data_probCol_colnamesFullfillCondition_list <- lapply(
              1:length(node),
              FUN = function(nodeN) {
                ## for each colname
                sapply(data_probCol_colnames,
                  FUN = function(state) {
                    if (value[nodeN] == 1) {
                    ...
                    if (value[nodeN] == 0) {
                      return(!(node[nodeN] %in% unlist(
                        strsplit(gsub("Prob.", "", gsub(".$", "", state)), split = "\\.\\.")
                      )))
                    }
                  }
                )
          ...
          data_probCol_colnamesFullfillCondition <-
            data_probCol_colnames[Reduce('*', data_probCol_colnamesFullfillCondition_list) > 0]
    ...
    conditions_proba_df <-
      data.frame(Time = data[, "Time"], do.call(cbind, conditions_proba_list),
                 TH = data[, "TH"])
...
for (j in 1:length(WT)) {
  folder <- WT[j]
  file <-
    list.files(paste0(folder, "/")) %>% grep("_probtraj_table.csv", ., value = TRUE)
  ...
  a1$drug <- NA
  a1$dose <- NA
  a1$RNA <- ifelse(grepl("mutRNA", file) == T, "mutRNA",
                   ifelse(grepl("RNA", file) == T, "RNA", NA))
  a1$activ <- ifelse(grepl("_AR_EGF", file) == T,"AR_EGF",
                     ifelse(grepl("_AR", file) == T,"AR",
...
drugs <-
  list.dirs(path = "./", recursive = F) %>% grep("LNCAP", ., value = TRUE) %>% sub("./", "", .)
      ...
      for (f in folder) {
        ...
        for (l in 1:length(dosage)) {
          for (m in 1:length(dosage)) {
            ...
            prefix <-paste0(sub("drugsim_", "", sub(pattern, "", f, perl = T)),
                "_",drugnames[j],dosage[l],"_",drugnames[k],dosage[m])
            print(prefix)
            res <-read.table(paste0("./", f, "/", prefix, "_probtraj_table.csv"),
                header = TRUE,
                sep = "\t",
                stringsAsFactors = FALSE
              ) %>% .[nrow(.), , drop = FALSE]
            ...
            a1$RNA <- ifelse(grepl("mutRNA", prefix) == T,"mutRNA",
                             ifelse(grepl("RNA", prefix) == T, "RNA", NA))
            a1$activ <-ifelse(grepl(paste0(a1$RNA, "_AR_EGF_"), prefix) == T,"AR_EGF",
                     ifelse(grepl(paste0(a1$RNA, "_AR_"), prefix) == T,"AR",
                       ifelse(grepl(paste0(a1$RNA, "_EGF_"), prefix) == T,"EGF",
                         ifelse(grepl(paste0(a1$RNA, "_00_"), prefix) == T, "00", NA)
                       )))
            a2 <- rbind(a2, a1)
          }
        }
...
drug_pheno_norm2 <- merge(rbind(
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "00" & drug_pheno$RNA == "RNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "00" & WT_pheno$RNA == "RNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "00" & drug_pheno$RNA == "mutRNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "00" & WT_pheno$RNA == "mutRNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "AR_EGF" & drug_pheno$RNA == "RNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "AR_EGF" & WT_pheno$RNA == "RNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "AR_EGF" & drug_pheno$RNA == "mutRNA",][,-c(1, 12:ncol(drug_pheno))]),
      2, as.matrix(WT_pheno[WT_pheno$activ == "AR_EGF" & WT_pheno$RNA == "mutRNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "AR" & drug_pheno$RNA == "RNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "AR" & WT_pheno$RNA == "RNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "AR" & drug_pheno$RNA == "mutRNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "AR" & WT_pheno$RNA == "mutRNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "EGF" & drug_pheno$RNA == "RNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "EGF" & WT_pheno$RNA == "RNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    ))),
    as.data.frame((sweep(
      as.matrix(drug_pheno[drug_pheno$activ == "EGF" & drug_pheno$RNA == "mutRNA",][,-c(1, 12:ncol(drug_pheno))]), 
      2, as.matrix(WT_pheno[WT_pheno$activ == "EGF" & WT_pheno$RNA == "mutRNA",][1,-c(1, 12:ncol(WT_pheno))]), `-`
    )))
  ),
...
drug_pheno_large <- drug_pheno_norm2 %>%
  ...
  mutate(., dose1 = as.numeric(dose1), dose2 = as.numeric(dose2)) %>% mutate(., value2 = scale(.$value))
